...
export const handleResellOnPoshmark = async (pieceInfo) => {
    try {
        ...
        const response = await fetch('https://' + POSHMARK_CONFIG.URL + '/api/prospect_posts?pk=' + POSHMARK_CONFIG.PARTNER_KEY, {
            method: 'POST',
        ...
        const redirectUrl = `https://${POSHMARK_CONFIG.URL}/resell?id=${id}&pmrd[url]=/create-listing?resell-id=${id}&utm_source=pm_resell&utm_campaign=fitted`;
